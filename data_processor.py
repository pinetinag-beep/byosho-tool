...
def load_mhlw_shisetsu(file_bytes: bytes) -> pd.DataFrame:
    ...
    # ── スタッフ数 ──
    # 常勤列をキーワードで探し、その右隣列を非常勤（常勤換算）として取得
    # Excel上: 常勤列, 非常勤列 が各職種で交互に並ぶ
    _staff_pairs = [
        (["施設全体_医師_常勤",        "常勤_医師",        "常勤医師"],        "常勤医師数",          "非常勤医師数"),
        (["施設全体_看護師_常勤",      "常勤_看護師",      "常勤看護師"],      "常勤看護師数",        "非常勤看護師数"),
        (["施設全体_理学療法士_常勤",  "常勤_理学療法士",  "理学療法士_常勤",  "常勤理学療法士"],     "常勤理学療法士数",    "非常勤理学療法士数"),
        (["施設全体_作業療法士_常勤",  "常勤_作業療法士",  "作業療法士_常勤",  "常勤作業療法士"],     "常勤作業療法士数",    "非常勤作業療法士数"),
        (["施設全体_言語聴覚士_常勤",  "常勤_言語聴覚士",  "言語聴覚士_常勤",  "常勤言語聴覚士"],     "常勤言語聴覚士数",    "非常勤言語聴覚士数"),
        (["施設全体_薬剤師_常勤",      "常勤_薬剤師",      "薬剤師_常勤",      "常勤薬剤師"],         "常勤薬剤師数",        "非常勤薬剤師数"),
        (["施設全体_診療放射線技師_常勤", "常勤_診療放射線技師", "診療放射線技師_常勤", "常勤診療放射線技師"], "常勤診療放射線技師数", "非常勤診療放射線技師数"),
    ...
    for kw in ["救急車の受入件数", "救急搬送"]:
        if "救急搬送件数" not in keep:
            col = _find_col(_col_list, kw)
            if col:
                keep["救急搬送件数"] = pd.to_numeric(df[col], errors="coerce").fillna(0).astype(int)
