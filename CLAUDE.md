...
各学会（外科学会・内科学会等）が公表する「専門医研修施設」「認定施設」等の一覧を取り込み、既存の病院データと医療機関名で突合して、条件検索で「◯◯学会認定施設で絞り込む」ができるようにする機能。2026年7月、内科学会・内分泌学会・眼科学会分で実装。
...
- **医療機関名の突合ロジック**（`_normalize()` + `_match()`）: 病床機能報告データ（`data_cache.parquet`、全年度分の表記ゆれを含めて逆引き辞書化）に対し、①法人格プレフィックス除去＋全角正規化した完全一致、②同一都道府県内でのサフィックス一致（学会データが法人名を省略している/病床機能報告側が法人名を含む、等の片側だけ付加語があるケースを吸収。4文字未満の短い名称同士は誤マッチ防止のため対象外）の2段階でマッチングする。実測一致率は内科学会78.0%（基幹施設のみなら97.6%、特別連携施設は56.5%——多くは病床機能報告の対象外の無床診療所等のため一致しないのが正常）、内分泌学会92.3%、眼科学会89.9〜93.9%。**東京慈恵会医科大学附属病院（本院）は3学会すべてで一致しない**ことを確認済みで、ユーザーからも「病床機能報告に記載がなく恒久的に突合できない」と確認が取れている（＝突合ロジックの不具合ではなく、この施設が`data_cache.parquet`に元々存在しない恒久的なデータ欠落。今後別の学会を追加した際にも同じ病院が未一致リストに出てきたら、この既知の欠落だと判断してよい）。
