...
各学会（外科学会・内科学会等）が公表する「専門医研修施設」「認定施設」等の一覧を取り込み、既存の病院データと医療機関名で突合して、条件検索で「◯◯学会認定施設で絞り込む」ができるようにする機能。2026年7月、内科学会・内分泌学会分で実装。
