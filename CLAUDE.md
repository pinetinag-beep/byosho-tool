...
## 学会認定施設データ（2026年7月着手・データ未着手）

各学会（外科学会・内科学会等）が公表する「専門医研修施設」「認定施設」等の一覧を取り込み、既存の病院データと医療機関名で突合して、条件検索で「◯◯学会認定施設で絞り込む」ができるようにする計画。

- **生データの置き場**: `gakkai_nintei_raw/`（ユーザーが学会ごとにファイルを追加していく運用。詳細は同フォルダの README.md 参照）
- **現状**: フォルダを用意しただけで、実データはまだ1件も無い。学会ごとに公表形式（Excel/PDF/Webページ）がバラバラな可能性が高いため、**必ず最初の1学会分のサンプルファイルを確認してからパーサーを設計すること**。施設基準届出データで学会・厚生局ごとに全く違う形式（Excel24列 / PDF届出受理名簿 / 差分PDF等）に苦労した教訓があるため、複数学会分を見ずに共通パーサーを先に書かない。
- **医療機関名の突合**: 完全一致は期待できない前提で、既存の `_normalize_hospital_for_match()`（法人格プレフィックス除去・全角正規化）を再利用しつつ、学会データ側の名称に独自の癖がないか都度確認する。可能であれば都道府県・住所等、名前以外の照合キーも学会データから取得しておくと同名異院の誤マッチを防げる（施設基準届出データの `都道府県コード + 医療機関名_正規化` キー方式を踏襲）。
- **想定する保存先**: `gakkai_nintei_cache.parquet`（未作成）。学会名・施設名・都道府県等を持たせ、複数学会に認定されている施設は学会ごとに行を分ける設計を想定（施設基準届出データと同様、1施設1行ではなく1施設×1認定=1行の縦持ち）。
- **UI**: 条件検索（`_view_mode == "search"`）に学会認定施設タブ／絞り込みを追加予定。既存の「施設基準届出」タブと同様、複数選択でOR/AND切り替えができるとよい（要件は今後詰める）。
