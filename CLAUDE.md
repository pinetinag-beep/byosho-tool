...
- **学会ごとにPDF形式が全く異なる（重要）**。新しい学会を追加する際は、既存の抽出関数が流用できると思い込まず、必ず該当PDFの構造を先に確認すること。
  ...
  - **眼科学会**（`gakkai_nintei_raw/眼科学会/`）: 「一般研修施設.pdf」（870件）は「県名／研修施設番号／研修施設名称」の3列表で抽出は容易だが、**県名が「青森」のようにサフィックス（都/道/府/県）省略形で記載されている**ため、`_normalize_pref_suffix()` で正式名称に変換してから突合する必要がある（これをせずに都道府県名で突合すると、北海道以外ほぼ全滅する）。「基幹研修施設.pdf」は**画像として保存されたPDF（テキスト層なし、`extract_text()`が空になる）で現状未対応**。OCRが必要になるが医療機関名の認識精度に不安があり、対応するかはユーザーと要相談。
