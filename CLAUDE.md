...
- **UI**: 条件検索（`_view_mode == "search"`）の絞り込みタブに「🎓 学会認定施設」を追加（`_tab_gakkai`）。学会ごとにマルチセレクトで区分を選べる（複数選択でOR）。フィルタリングは `医療機関コード` で直接絞り込む（施設基準届出フィルターのような`.apply()`での名称照合は不要、突合済みのコードを使うだけなので高速）。
