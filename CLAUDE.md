...
各学会（外科学会・内科学会等）が公表する「専門医研修施設」「認定施設」等の一覧を取り込み、既存の病院データと医療機関名で突合して、条件検索で「◯◯学会認定施設で絞り込む」ができるようにする機能。2026年7月、内科学会分で初実装。

- **生データの置き場**: `gakkai_nintei_raw/<学会名>/`（ユーザーが学会ごとにファイルを追加していく運用）
- **内科学会の形式**（`gakkai_nintei_raw/内科学会/`）: 基幹施設・連携施設・特別連携施設の3区分がそれぞれ別PDF（`kikan_2026.pdf` / `renkei_2026.pdf` / `t_renkei_2026.pdf`）。中身はページ単位で「都道府県／施設名」の2列表が並ぶだけのシンプルな構造で、`pdfplumber` の `extract_tables()` がそのまま使える（施設基準届出PDFのような複雑な複数ページレコード結合は不要）。**他学会を追加する際は形式が異なる前提で、必ず該当PDFの構造を先に確認すること。**
- **ビルドスクリプト**: `build_gakkai_nintei.py`。`_SOCIETY_FILES` 辞書に学会名→[(ファイル名, 区分ラベル), ...] を追記していく方式。実行すると `gakkai_nintei_cache.parquet` を再生成する。
- **医療機関名の突合ロジック**（`_normalize()` + `_match()`）: 病床機能報告データ（`data_cache.parquet`、全年度分の表記ゆれを含めて逆引き辞書化）に対し、①法人格プレフィックス除去＋全角正規化した完全一致、②同一都道府県内でのサフィックス一致（学会データが法人名を省略している/病床機能報告側が法人名を含む、等の片側だけ付加語があるケースを吸収。4文字未満の短い名称同士は誤マッチ防止のため対象外）の2段階でマッチングする。内科学会データでの実測一致率は**78.0%**（3,234件中2,524件）。基幹施設（≒大規模病院）だけなら97.6%と高いが、特別連携施設（クリニック・医務室等を含む）は56.5%程度に下がる——多くは病床機能報告の対象外（無床診療所等）のため一致しないのが正常（施設基準届出データの有床診療所一致率が低かったのと同じ構造）。
- **保存先スキーマ**（`gakkai_nintei_cache.parquet`）: `学会名` / `区分` / `都道府県名` / `施設名` / `施設名_正規化` / `医療機関コード`（マッチできた場合のみ値あり）/ `マッチ状態`（"一致"/"未一致"）。1施設1行ではなく「1施設×1区分＝1行」の縦持ち（同じ施設が複数区分に認定されるケースは考慮していないが、内科学会の3区分は排他的なので問題なし）。
